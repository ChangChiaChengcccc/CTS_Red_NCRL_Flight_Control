...
/* distributed control */
MAT_ALLOC(Gamma, 4, 8);
MAT_ALLOC(Gamma_trans, 8, 4);
MAT_ALLOC(H, 8, 8);
MAT_ALLOC(u_s, 4, 1);
MAT_ALLOC(u_c_star, 8, 1);
MAT_ALLOC(HG_t, 8, 4);
MAT_ALLOC(GH, 4, 8);
MAT_ALLOC(GHG_t, 4, 4);
MAT_ALLOC(GHG_t_inv, 4, 4);
MAT_ALLOC(HG_tGHG_t_inv, 8, 4);
...
void geometry_ctrl_init(void)
{
	...
	/* distributed control */
	MAT_INIT(Gamma, 4, 8);
	MAT_INIT(Gamma_trans, 8, 4);
	MAT_INIT(H, 8, 8);
	MAT_INIT(u_s, 4, 1);
	MAT_INIT(u_c_star, 8, 1);
	MAT_INIT(HG_t, 8, 4);
	MAT_INIT(GH, 4, 8);
	MAT_INIT(GHG_t, 4, 4);
	MAT_INIT(GHG_t_inv, 4, 4);
	MAT_INIT(HG_tGHG_t_inv, 8, 4);
...
void mr_geometry_ctrl_thrust_allocation(float *moment, float total_force)
{
	...
	// Gamma
	float r = 0.403;
	mat_data(Gamma)[0*8 + 0] = 1;
	mat_data(Gamma)[1*8 + 0] = r;
	mat_data(Gamma)[2*8 + 0] = 0;
	mat_data(Gamma)[3*8 + 0] = 0;
	mat_data(Gamma)[0*8 + 1] = 0;
	mat_data(Gamma)[1*8 + 1] = 1;
	mat_data(Gamma)[2*8 + 1] = 0;
	mat_data(Gamma)[3*8 + 1] = 0;
	mat_data(Gamma)[0*8 + 2] = 0;
	mat_data(Gamma)[1*8 + 2] = 0;
	mat_data(Gamma)[2*8 + 2] = 1;
 	mat_data(Gamma)[3*8 + 2] = 0;
	mat_data(Gamma)[0*8 + 3] = 0;
	mat_data(Gamma)[1*8 + 3] = 0;
	mat_data(Gamma)[2*8 + 3] = 0;
	mat_data(Gamma)[3*8 + 3] = 1;
	mat_data(Gamma)[0*8 + 4] = 1;
	mat_data(Gamma)[1*8 + 4] = -r;
	mat_data(Gamma)[2*8 + 4] = 0;
	mat_data(Gamma)[3*8 + 4] = 0;
	mat_data(Gamma)[0*8 + 5] = 0;
	mat_data(Gamma)[1*8 + 5] = 1;
	mat_data(Gamma)[2*8 + 5] = 0;
	mat_data(Gamma)[3*8 + 5] = 0;
	mat_data(Gamma)[0*8 + 6] = 0;
	mat_data(Gamma)[1*8 + 6] = 0;
	mat_data(Gamma)[2*8 + 6] = 1;
	mat_data(Gamma)[3*8 + 6] = 0;
	mat_data(Gamma)[0*8 + 7] = 0;
	mat_data(Gamma)[1*8 + 7] = 0;
	mat_data(Gamma)[2*8 + 7] = 0;
	mat_data(Gamma)[3*8 + 7] = 1;
	// H
	mat_data(H)[0*8 + 0] = 1;
	mat_data(H)[1*8 + 0] = 0;
	mat_data(H)[2*8 + 0] = 0;
	...
	mat_data(H)[7*8 + 7] = 1;
	// u_s
	mat_data(u_s)[0] = total_force;
	mat_data(u_s)[1] = moment[0];
	mat_data(u_s)[2] = moment[1];
	mat_data(u_s)[3] = moment[2];
	// u_c_star = H^-2 * Gamma_trans * (Gamma*H^-2*Gamma_trans)^-1 * u_s
	MAT_TRANS(&Gamma,&Gamma_trans);
	MAT_MULT(&H,&Gamma_trans,&HG_t);
	MAT_MULT(&Gamma,&H,&GH);
	MAT_MULT(&GH,&Gamma_trans,&GHG_t);
	MAT_INV(&GHG_t,&GHG_t_inv);
	MAT_MULT(&HG_t,&GHG_t_inv,&HG_tGHG_t_inv);
	MAT_MULT(&HG_tGHG_t_inv,&u_s,&u_c_star);

	/* quadrotor thrust allocation */
	float distributed_force = mat_data(u_c_star)[4] * 0.25; //split force to 4 motors
	float motor_force[4];
	motor_force[0] = -l_div_4 * mat_data(u_c_star)[5] + l_div_4 * mat_data(u_c_star)[6] +
	                 -b_div_4 * mat_data(u_c_star)[7] + distributed_force;
	motor_force[1] = +l_div_4 * mat_data(u_c_star)[5] + l_div_4 * mat_data(u_c_star)[6] +
	                 +b_div_4 * mat_data(u_c_star)[7] + distributed_force;
	motor_force[2] = +l_div_4 * mat_data(u_c_star)[5] - l_div_4 * mat_data(u_c_star)[6] +
	                 -b_div_4 * mat_data(u_c_star)[7] + distributed_force;
	motor_force[3] = -l_div_4 * mat_data(u_c_star)[5] - l_div_4 * mat_data(u_c_star)[6] +
	                 +b_div_4 * mat_data(u_c_star)[7] + distributed_force;
	...
	// u_c_star = H^-2 * Gamma_trans * (Gamma* H^-2* Gamma_trans)^-1 * u_s
	MAT_TRANS(&Gamma,&Gamma_trans); //4*8 --> 8*4
	MAT_MULT(&H,&Gamma_trans,&HG_t); // 8*8 x 8*4 = 8*4
	MAT_MULT(&Gamma,&H,&GH); // 4*8 x 8*8 = 4*8
	MAT_MULT(&GH,&Gamma_trans,&GHG_t); // 4*8 x 8*4 = 4*4
	MAT_INV(&GHG_t,&GHG_t_inv); // 4*4 --> 4*4
	MAT_MULT(&HG_t,&GHG_t_inv,&HG_tGHG_t_inv); //8*4 x 4*4 = 8*4
	MAT_MULT(&HG_tGHG_t_inv,&u_s,&u_c_star); // 8*4 x 4*1 = 8*1
